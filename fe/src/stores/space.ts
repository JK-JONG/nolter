...
export const useSpace = defineStore('space', () => {
  ...
  function reset() { syncCode.value = ''; nickname.value = ''; rooms.value = []; version.value = 0 }

  // ── 프로필 ──
  function setProfile(n: string, c: UserColorKey) {
    if (!isValidNickname(n)) return
    nickname.value = n.trim(); colorKey.value = c
    push()
  }
  ...
  return {
    syncCode, nickname, colorKey, rooms, color,
    hasSync, hasProfile, ready, isValidNickname,
